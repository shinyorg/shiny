...
public class ScreenRecorderService : ShinyAndroidForegroundService
{
    ...
    public override StartCommandResult OnStartCommand(Intent? intent, StartCommandFlags flags, int startId)
    {
        ...
        // a Stop tapped on a notification that outlived its recording must not leave a started service
        // behind - Stop() rather than StopSelf so OnStop clears IsStarted and the next recording starts
        // the service again instead of assuming it is still up
