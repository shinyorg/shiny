...
/// <summary>
/// Base for the calendar <see cref="AIFunction"/> tools. Holds the name/description/schema and
...
abstract class CalendarAIFunctionBase : AIFunction
{
    ...
    protected static bool? GetBool(AIFunctionArguments args, string key)
    {
        if (!args.TryGetValue(key, out var raw) || raw is null)
            return null;
        switch (raw)
        {
            ...
            case JsonElement el when el.ValueKind is JsonValueKind.True or JsonValueKind.False: return el.GetBoolean();
            case string s when bool.TryParse(s, out var pb): return pb;
            default: return null;
        }
    }
