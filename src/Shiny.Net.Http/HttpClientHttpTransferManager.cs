...
) : IHttpTransferManager, IShinyStartupTask, IDisposable
{
    ...
    public Task Pause(string identifier)
    {
        ...
        {
            // The process loop watches for this status change and interrupts the in-flight
            // request (if any) without removing the transfer. Downloads keep their partial
            // file for range-resume; uploads restart on resume.
